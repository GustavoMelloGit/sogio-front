...
const specularButtonVariants = cva(
  cn(
    'relative inline-flex items-center justify-center',
    'm-0 border-0',
    '[font-family:inherit] font-medium tracking-[0.01em] leading-none',
    'text-(--sb-text-color)',
    'bg-[color-mix(in_srgb,var(--sb-tint)_calc(var(--sb-tint-opacity)*100%),transparent)]',
    'rounded-(--sb-radius)',
    'backdrop-blur-(--sb-blur)',
    'shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_8px_24px_rgba(0,0,0,0.25)]',
    'cursor-pointer outline-hidden',
    'transition-transform duration-150 [transition-timing-function:ease]',
    'active:scale-[0.97] disabled:active:scale-100',
    'focus-visible:outline-2 focus-visible:outline-offset-[3px]',
    'focus-visible:outline-[color-mix(in_srgb,var(--sb-text-color)_60%,transparent)]',
...
export type SpecularButtonProps = VariantProps<
  typeof specularButtonVariants
> & {
  children?: ReactNode;
  radius?: number;
  tint?: string;
...
/**
 * WebGL (ogl) specular sweep behind the button edge — see reactbits.dev.
 * Ported to Tailwind + CSS custom properties instead of a companion .css
 * file so it fits this project's styling convention; the shader/animation
 * logic below is unchanged from the source component.
 */
export const SpecularButton: FC<SpecularButtonProps> = ({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={cn(specularButtonVariants({ size }), className)}
      style={style}
    >
      <span
        ref={fxRef}
